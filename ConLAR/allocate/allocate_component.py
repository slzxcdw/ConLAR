...
import numpy as np
class AllocateComponent(Thread):
	def __init__(self):
		...
		self._stop = Event()
	        ...
	        self.p = 0.05
                ...
                self.__thre_response = self.__config.thre_response
                self.__cur_response = self.__config.cur_response
                self.__predict_path = self.__config.predict_path
                self.__vio_path = self.__config.vio_path
                self.__name = self.__config.name
                self.__cfg = self.__config.cfg
	def stop(self):
		self._stop()
	...
	def run(self):
		...
		while not self.stopped():
                     target = self.__thre_response
                     avgpath = self.__cur_response
                     vio_path = self.__vio_path
                     afile = open(vio_path,"r")
                     lines = afile.readlines()
                     p = 0 
                     if len(lines) == 1:
                         p = (float)(lines[0])
                     cfile = open(avgpath,"r")
                     lines = cfile.readlines()
                     nowv = 10000 
                     if len(lines) == 1:
                         nowv = (float)(lines[0])
                     fullpath = self.__predict_path
